...
export async function flyLevel(autopilot, flightInformation) {
  ...
  const { aileron, speed, turnRate, bank, upsideDown } = flightData;
  ...
  // Smooth out the stick changes with some lerping
  const oldAileron = (aileron / 100) * -16384;
  const newAileron = offset + proportion * maxStick;
  const mixed = lerp(0.5, oldAileron, newAileron);
  return api.trigger("AILERON_SET", mixed | 0);
}
